...
if __name__ == '__main__':
    ...
    for i in range(int(input())):
        ...
        all_grades.append([name, score])
        
    # Using sort(key = ...) to sort list by sublist second value
    # PY KEYS
    all_grades.sort(key = lambda x: x[1])
    
    # removing lowest value so second to last grade is new minimum
    
    all_grades.remove(min(all_grades, key = lambda x: x[1]))
    
    low_grade2 = min(all_grades, key = lambda x: x[1])[1]
    low_names2 = [k[0] for k in all_grades if k[1] == low_grade2]
    low_names2.sort()
